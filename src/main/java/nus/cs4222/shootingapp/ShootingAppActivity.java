...
public class ShootingAppActivity 
    ...
    private void processGravityValues( SensorEvent event ) {
        ...
        // Store last gravity readings for rotation matrix calculation
        gravityForCompass = gravityValues;
    ...
    private void detectShootingDirectionAndRegion( SensorEvent event ) {
        ...
        // output array for getOrientation method
        float[] orientation = new float[3];

        // helper arrays for getRotationMatrix method
        ...
        if (sensorManager.getRotationMatrix(rotationMatrix, inclinationMatrix, accelerationSum, geomagneticValues)) {
            orientation = sensorManager.getOrientation(rotationMatrix, orientation);
        }

        // orientation[0] holds azimuth
        float radianAngle = orientation[0];

        // Convert to degrees
        float rawShootingDirection = (float) Math.toDegrees( radianAngle );
        shootingDirection = rawShootingDirection + (float)180;

        if(shootingDirection < 45.0)
            shootingRegion = 1;
        else if (shootingDirection < 90.0)
            shootingRegion = 2;
        else if (shootingDirection < 135.0)
            shootingRegion = 3;
        else if (shootingDirection < 180.0)
            shootingRegion = 4;
        else if (shootingDirection < 225.0)
            shootingRegion = 5;
        else if (shootingDirection < 270.0)
            shootingRegion = 6;
        else if (shootingDirection < 315.0)
            shootingRegion = 7;
        else if (shootingDirection < 360.0)
            shootingRegion = 8;
    ...
    private void processAcclValues( SensorEvent event ) {
        ...
        // store linear acceleration values for calculation of rotational matrix
        linearAccForCompass = event.values.clone();
    ...
    // arrays to store latest linear Acc and gravity readings
    // to get rotational matrix
    private float[] gravityForCompass = new float[3];
    private float[] linearAccForCompass = new float[3];
